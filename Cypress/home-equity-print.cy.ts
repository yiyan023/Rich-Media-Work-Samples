describe('PDF Download - Home Equity', () => {
  it('Successfully Downloaded PDF for User and Spouse (Home Equity)', () => {
    ...
    // press TD Home Equity Flexline Option
    shadow().find('input#btn-flexline').should('have.css', 'background-color').and('equal','rgba(0, 0, 0, 0)');
    shadow().find("button.btn.td-btn-primary-light").should('be.disabled')
    shadow().find("input#btn-flexline").click()
    shadow().find('input#btn-flexline').should('have.css', 'background-color').and('equal','rgb(0, 138, 0)');
